...
function Profile() {
	...
	const [studeyYear, setStudyYear] = useState();
	...
	useEffect(() => {
		...
		Axios.get(`https://localhost:44328/api/Students/${id}`).then((response) => {
			...
			Axios.get(`https://localhost:44328/student/${studentId}`).then((response) => {
				...
				setSubjectGrades(JSON.parse(localStorage.getItem("grades")));
			});
		});
	}, []);
	...
	if (typeof subjectGrades !== "undefined") {
		...
		var average = (gradesArray.reduce((a, v) => (a = a + v), 0) / Object.keys(subjectGrades).length).toFixed(3);
	}
